...
import {
  getSettings, saveSettings, getPortfolio, savePosition, getRegistry, recordHealedSelector,
...
test('settings fall back to the documented defaults and merge on save', async () => {
  ...
  await saveSettings({ apiKey: 'sk-ant-x' });
  const settings = await getSettings();
  assert.equal(settings.apiKey, 'sk-ant-x');
