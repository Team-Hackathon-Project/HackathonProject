...
```bash
npm install && npm run check     # 129 tests, static bundle validation
```

Load it: `chrome://extensions` (or `edge://extensions`) → **Developer mode** →
**Load unpacked** → pick this folder. Optionally paste an Anthropic API key in
the options page; the demo works without one, it just loses self-healing and
falls back to the local rules engine for advisories.
...
Say: *the key is stored locally and only ever sent to `api.anthropic.com`. The
healed-selector registry is inspectable and resettable, and the repair log
...
- `npm run check` — 129 unit/integration tests plus static manifest and module
  graph validation.
